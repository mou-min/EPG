...
echo "https://fastly.jsdelivr.net/gh/mou-min/EPG/colo 另存为 colo.txt"
echo "https://fastly.jsdelivr.net/gh/mou-min/EPG/url 另存为 url.txt"
echo "https://fastly.jsdelivr.net/gh/mou-min/EPG/ips-v4_c 另存为 ips-v4.txt"
echo "https://fastly.jsdelivr.net/gh/mou-min/EPG/ips-v6 另存为 ips-v6.txt"
while true
do
	if [ ! -f "colo.txt" ]
	then
		echo "从服务器下载数据中心信息 colo.txt"
		curl --retry 2 -s https://fastly.jsdelivr.net/gh/mou-min/EPG/colo -o colo.txt
	elif [ ! -f "url.txt" ]
	then
		echo "从服务器下载测速文件地址 url.txt"
		curl --retry 2 -s https://fastly.jsdelivr.net/gh/mou-min/EPG/url -o url.txt
	elif [ ! -f "ips-v4.txt" ]
	then
		echo "从服务器下载IPV4数据 ips-v4.txt"
		curl --retry 2 -s https://fastly.jsdelivr.net/gh/mou-min/EPG/ips-v4_c -o ips-v4.txt
	elif [ ! -f "ips-v6.txt" ]
	then
		echo "从服务器下载IPV6数据 ips-v6.txt"
		curl --retry 2 -s https://fastly.jsdelivr.net/gh/mou-min/EPG/ips-v6 -o ips-v6.txt
	else
		break
	fi
done
